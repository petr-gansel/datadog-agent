...
func ExecJMXCommandConsole(wmeta workloadmeta.Component, command string, selectedChecks []string, logLevel string, configs []integration.Config) error {
	return execJmxCommand(wmeta, command, selectedChecks, jmxfetch.ReporterConsole, log.JMXInfo, logLevel, configs)
}
...
// of the Agent.
// The common utils, including AutoConfig, must have already been initialized.
func ExecJmxListWithMetricsJSON(wmeta workloadmeta.Component, selectedChecks []string, logLevel string, configs []integration.Config) error {
	// don't pollute the JSON with the log pattern.
	out := func(a ...interface{}) {
		fmt.Println(a...)
	}
	return execJmxCommand(wmeta, "list_with_metrics", selectedChecks, jmxfetch.ReporterJSON, out, logLevel, configs)
}
...
// of the Agent.
// The common utils, including AutoConfig, must have already been initialized.
func ExecJmxListWithRateMetricsJSON(wmeta workloadmeta.Component, selectedChecks []string, logLevel string, configs []integration.Config) error {
	// don't pollute the JSON with the log pattern.
	out := func(a ...interface{}) {
		fmt.Println(a...)
	}
	return execJmxCommand(wmeta, "list_with_rate_metrics", selectedChecks, jmxfetch.ReporterJSON, out, logLevel, configs)
}

// execJmxCommand runs the provided JMX command name on the selected checks.
// The common utils, including AutoConfig, must have already been initialized.
func execJmxCommand(wmeta workloadmeta.Component, command string, selectedChecks []string, reporter jmxfetch.JMXReporter, output func(...interface{}), logLevel string, configs []integration.Config) error {
	// start the cmd HTTP server
	if err := api.StartServer(nil, nil, nil, nil, nil, wmeta, util.NewNoneOptional[logsAgent.Component]()); err != nil {
