	...
	for idx := range allConfigs {
		...
		if check.IsJMXConfig(*conf) {
			...
			if cliParams.checkRate {
				if err := standalone.ExecJmxListWithRateMetricsJSON(wmeta, selectedChecks, config.GetString("log_level"), allConfigs); err != nil {
					return fmt.Errorf("while running the jmx check: %v", err)
				}
			} else {
				if err := standalone.ExecJmxListWithMetricsJSON(wmeta, selectedChecks, config.GetString("log_level"), allConfigs); err != nil {
					return fmt.Errorf("while running the jmx check: %v", err)
				}
			}
